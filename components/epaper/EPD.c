...
{
	// upper right
	if (option & EPD_ELLIPSE_UPPER_RIGHT)
		_drawPixel(x0 + x, y0 - y, color);
	// upper left
	if (option & EPD_ELLIPSE_UPPER_LEFT)
		_drawPixel(x0 - x, y0 - y, color);
	// lower right
	if (option & EPD_ELLIPSE_LOWER_RIGHT)
		_drawPixel(x0 + x, y0 + y, color);
	// lower left
	if (option & EPD_ELLIPSE_LOWER_LEFT)
		_drawPixel(x0 - x, y0 + y, color);
}
...
static int rotatePropChar(int x, int y, int offset)
{
	...
	for (int j = 0; j < fontChar.height; j++)
	{
		for (int i = 0; i < fontChar.width; i++)
		{
			...
			if ((ch & mask) != 0)
				_drawPixel(newX, newY, _fg);
			else if (!font_transparent)
				_drawPixel(newX, newY, _bg);
...
void EPD_print(char *st, int x, int y)
{
	...
	if (x == RIGHT)
		x = dispWin.x2 - tmpw + dispWin.x1 - 5;
	else if (x == CENTER)
		x = (((dispWin.x2 - dispWin.x1 + 1) - tmpw) / 2) + dispWin.x1;

	if (y == BOTTOM)
		y = dispWin.y2 - fh + dispWin.y1 - 3;
	else if (y == CENTER)
	...
	if (cfont.x_size != 0)
	{
		...
		EPD_OFFSET = 0; // fixed font; offset not needed

	if ((EPD_Y + tmph - 1) > dispWin.y2)
		return;
	...
	for (i = 0; i < stl; i++)
	{
		...
		{ // === '\r', erase to eol ====
			if ((!font_transparent) && (font_rotate == 0))
				_fillRect(EPD_X, EPD_Y, dispWin.x2 + 1 - EPD_X, tmph, _bg);
		}
		...
		{ // ==== other characters ====
			...
			if (cfont.x_size == 0)
			{
				// == proportional font
				if (font_rotate == 0)
					EPD_X += printProportionalChar(EPD_X, EPD_Y) + 1;
				else
				{
...
void EPD_setclipwin(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
	...
	if (dispWin.x2 >= EPD_DISPLAY_WIDTH)
		dispWin.x2 = EPD_DISPLAY_WIDTH - 1;
	if (dispWin.y2 >= EPD_DISPLAY_HEIGHT)
